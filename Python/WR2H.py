...
#outdirplot=print os.environ["outdirplot"]

if not os.path.exists(outdirplot):
    os.makedirs(outdirplot)
...
if ntime > 1:   # if we have a bulletin with time array ...
        vt,vtr,vtrx,vtry,cincl,sincl=vtcalc(time,lat,lon,vmax,inpfile,outdirplot) #get translational velocity
#	if (maxcc) exit 
...
plt.figure()
plt.plot(time,b)
plt.axis([0.,tmax,0.4,2.4]) 
plt.xlabel('Time [h]')
plt.ylabel('B')
plt.figtext(.01,.05,'TC '+np.str(hurName),horizontalalignment='left',size='small')
plt.figtext(.01,.03,"Holland's Parameter (B)",horizontalalignment='left',size='small')
plt.figtext(.01,.01,"Bulletin No."+np.str(bulNo)+' ('+np.str(bulDate)+')',horizontalalignment='left',size='small')
plt.subplots_adjust(bottom=.15)
plt.savefig(outdirplot+'/b.png')


plt.figure()
rmaxkm=rmax/1000
#setexp rmaxkm Rmax
plt.plot(time ,rmaxkm)
plt.axis([0.,tmax,0.,140.]) 
plt.xlabel('Time [h]') 
plt.ylabel('Rmax (km)') 
plt.figtext(.01,.05,'TC'+ np.str(hurName),horizontalalignment='left',size='small')
plt.figtext(.01,.03,'Radius of maximum wind (km)',horizontalalignment='left',size='small')
plt.figtext(.01,.01,'Bulletin No.'+np.str(bulNo)+' ('+np.str(bulDate)+')',horizontalalignment='left',size='small')
plt.subplots_adjust(bottom=.15)
plt.savefig(outdirplot+'/rmax.png')

plt.figure()
deltapmb=deltap/100.
#setexp deltapmb Deltap
plt.plot(time,deltapmb)
plt.axis([0.,tmax,0.,140.]) 
plt.xlabel('Time [h]') 
plt.ylabel('Deltap (mb)')
plt.figtext(.01,.05,'TC '+np.str(hurName),horizontalalignment='left',size='small')
plt.figtext(.01,.03,'Pressure Drop (mb)',horizontalalignment='left',size='small')
plt.figtext(.01,.01,'Bulletin No. '+np.str(bulNo)+' ('+np.str(bulDate)+')',horizontalalignment='left',size='small')
plt.subplots_adjust(bottom=.15)
plt.savefig(outdirplot+'/deltap.png')

plt.figure()
plt.plot(time,k)
plt.axis([0.,tmax,-.2,0.3]) 
plt.figtext(.01,.05,'TC '+np.str(hurName),horizontalalignment='left',size='small')
plt.figtext(.01,.03,'k parameter',horizontalalignment='left',size='small')
plt.figtext(.01,.01,'Bulletin No. '+np.str(bulNo)+' ('+np.str(bulDate)+')',horizontalalignment='left',size='small')
plt.subplots_adjust(bottom=.15)
plt.savefig(outdirplot+'/k.png')

plt.figure()
plt.plot(time,vmax0)
plt.axis([0.,tmax,0.,80.]) 
plt.xlabel('Time [h]') 
plt.ylabel('Velocity (m/s)')
plt.figtext(.01,.05,'TC '+np.str(hurName),horizontalalignment='left',size='small')
plt.figtext(.01,.03,'Vmax (m/s)',horizontalalignment='left',size='small')
plt.figtext(.01,.01,'Bulletin No. '+np.str(bulNo)+' ('+np.str(bulDate)+')',horizontalalignment='left',size='small')
plt.subplots_adjust(bottom=.15)
plt.savefig(outdirplot+'/vmax.png')
