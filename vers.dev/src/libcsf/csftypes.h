...
#ifndef lint
# define RCS_ID_CSFTYPES_H "$Header: /home/pcrtree/SRC.RCS/libs/csf/RCS/csftypes.h,v 2.4 1997/09/04 16:31:25 cees Exp $"
#endif
...
# else
#  define	IS_MV_REAL4(x)	((*((const UINT4 *)(x))) == MV_UINT4)
#  define	IS_MV_REAL8(x)	(((const UINT4 *)(x))[1] == MV_UINT4)
# endif
#else
# ifdef CPU_BIG_ENDIAN
#  ifdef CPU_LITTLE_ENDIAN
...
#define MV_INT1   ((INT1)0x80)
#define MV_INT2   ((INT2)0x8000)
#define MV_INT4   ((INT4)0x80000000L)
 
#define MV_UINT1  ((UINT1)0xFF)
#define MV_UINT2  ((UINT2)0xFFFF)
#define MV_UINT4  ((UINT4)0xFFFFFFFFL)
...
/* MV_REAL4 is a NAN with the same bitpattern as MV_UINT4 */
/* MV_REAL8 is a NAN with the same bitpattern as two MV_UINT4's */
/* only the first are already a NAN, so that's what we test */
 
/* x is a pointer to a value */
#define IS_MV_UINT1(x)	((*((const UINT1 *)x)) == MV_UINT1)
#define IS_MV_UINT2(x)	((*((const UINT2 *)x)) == MV_UINT2)
#define IS_MV_UINT4(x)	((*((const UINT4 *)x)) == MV_UINT4)
#define IS_MV_INT1(x)	((*((const INT1 *)x)) == MV_INT1)
#define IS_MV_INT2(x)	((*((const INT2 *)x)) == MV_INT2)
#define IS_MV_INT4(x)	((*((const INT4 *)x)) == MV_INT4)
...
#define	SET_MV_REAL4(x)	((*(UINT4 *)(x)) = MV_UINT4)
#define	SET_MV_REAL8(x)	SET_MV_REAL4((x)),SET_MV_REAL4((((UINT4 *)(x))+1))
...
/* PREVENT DOUBLE INCLUSION CSF__CSFTYPES */
#endif
